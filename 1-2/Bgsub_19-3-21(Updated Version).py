...
def press(button):
    if button == "Cancel":
        ...
        for video_path in filenames:
            ...
            print (temp)
            print("The current modified time is:-------->")
            print (time.strftime("%Y-%m-%d %H:%M:%S", temp))
            print("End of getting modified time--------->")
            #The m_time is correct
            #--------------------------------------------------

            '''
            d = time.strftime("%Y-%m-%d", temp)
            m = time.strftime("%B", temp)
            #h1 is the ending time of that video.
            h1 = time.strftime("%H", temp)
            #Date that the video is 
            #t = dateArr.strftime("%H:%M:%S")
            #t = time.strftime("%H:%M:%S", temp) 
            t = h1+':'+dateArr.strftime("%M:%S")
            #print('Hour1 is :',h1)
            ...
            d = time.strftime("%Y-%m-%d", temp)
            m = time.strftime("%B", temp)
            et = time.strftime("%H",temp)
            sec = int(math.ceil(length/fps))
            start_time = time.mktime(temp)-sec
            start_time2 = time.localtime(start_time)
            t = time.strftime("%H:%M:%S",start_time2)
            et_format = time.strftime("%H:%M:%S",temp)
            h1 = time.strftime("%H",start_time2)
            h2 = time.strftime("%H", temp)
            ...
            if ((count-BG)==0 or (BG/count)>0.7):
                write_data.append('Other')
            else:
            ...
            print('The writting hour1 is:', h1)
            if (int(h1) != int(h2)):
                write_data.append(h2)
            else:
                write_data.append('-')
            ...
            if (count-BG)==0:
                    #write_data.append('N')
                    write_data.append('*Need to be checked')
            if (count-BG) != 0:
