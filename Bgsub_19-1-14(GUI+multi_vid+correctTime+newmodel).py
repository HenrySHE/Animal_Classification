...
def press(button):
    if button == "Cancel":
        ...
        for video_path in filenames:
            ...
            if (count-BG) != 0:
                if ((max_val/(count-BG))*100 < 70):
                    write_data.append('N')
                    write_data.append(str((max_val/(count-BG))*100)+ '%')
                else:
                    write_data.append('Y')
                    write_data.append(str((max_val/(count-BG))*100)+ '%')
            ...
            # Write into scv file
            with open('data.csv','a') as csvfile:
                fieldnames = ['Video_name','Month','Date','Species','Abundance','Starting_Time','Ending_Time','Activity_Hour','Second_Hour','Activity_Time(Sec)','Remarkable_Behavior','Good_Shot?','Probability']
                writer = csv.writer(csvfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                #writer.writerow(fieldnames)
                writer.writerow(write_data)
